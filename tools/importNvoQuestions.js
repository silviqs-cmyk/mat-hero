...
function printSummary({ mode, inputPath, courseSlug, plan }) {
  const optionCount = plan.limitedReady.reduce((sum, item) => sum + item.options.length, 0);
  ...
  console.log(`Questions to import after limit: ${plan.limitedReady.length}`);
...
async function applyImport(supabase, plan) {
  ...
  for (const item of plan.limitedReady) {
...
async function main() {
  ...
  plan.limitedReady = args.limit === null ? plan.ready : plan.ready.slice(0, args.limit);
