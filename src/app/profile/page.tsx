...
export default async function ProfilePage() {
  ...
  const recommendation =
    weakestTopic?.topic
      ? `Повтори темата ${weakestTopic.topic.toLowerCase()} и после мини още веднъж през кратък тест.`
  ...
  const completedDayNumbers = Array.from(
    new Set(
      [
        ...
        ...effectiveResults.map((result) => result.dayNumber).filter((dayNumber): dayNumber is number => Boolean(dayNumber)),
      ],
    ),
  ).sort((left, right) => left - right);
  const derivedCurrentDay =
    completedDayNumbers.length > 0 ? Math.min(course.duration_days, Math.max(...completedDayNumbers) + 1) : 1;
  const currentDayValue = progress?.current_day_number ?? derivedCurrentDay;
  const xpValue =
    progress?.total_xp ??
    effectiveResults.reduce((sum, result) => sum + result.score + 25, 0);
  const streakValue = progress?.streak_days ?? effectiveResults.length;
  const completedDaysLabel = `${completedDayNumbers.length}/10`;
  ...
  return (
    <div className="space-y-5 lg:mx-auto lg:max-w-5xl">
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <ScoreCard title="Текущ ден" value={`${currentDayValue}`} helper="от 10" accent="cyan" />
        ...
        <div className="mt-4 space-y-3">
          {displayedWeakTopics.length > 0 ? (
            displayedWeakTopics.map((topic) => (
              <WeakTopicCard key={topic.topic} topic={topic.topic} score={topic.score} />
            ))
          ) : (
